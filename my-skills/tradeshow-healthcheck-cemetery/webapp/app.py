...
Ring Ring Marketing | Cemetery Edition

Wraps the trade show health check
(my-skills/tradeshow-healthcheck-cemetery) into a browser UI. Yvonne
enters cemetery name + cities, clicks Run, and watches the scan
unfold live in the page. When the scan finishes, the full scorecard
...
LOGO_PATH = ASSETS_DIR / "rrm_logo.jpg"
...
# Mirror anything in st.secrets into os.environ so the underlying scripts
# (which read env vars directly) pick them up. This must run before
# importing health_check.
# ----------------------------------------------------------------------------
...
# The packaged pdf_generator points RRM_LOGO_PATH at a Windows-only D:\
...
if LOGO_PATH.exists():
    pdf_generator.RRM_LOGO_PATH = str(LOGO_PATH)

_PDF_TMP_DIR = Path(tempfile.gettempdir()) / "rrm_cemetery_healthcheck_pdfs"
...
st.set_page_config(
    page_title="Cemetery Health Check | Ring Ring Marketing",
    page_icon="🪦",
    layout="centered",
)
...
st.markdown(
    ...
    <style>
      ...
      .stButton > button[kind="primary"] {{
        background-color: {RRM_RED};
        border-color: {RRM_RED};
        color: white;
        font-weight: 700;
        font-size: 1.05rem;
      ...
      .stButton > button[kind="primary"]:hover {{
        background-color: {RRM_DARK};
        border-color: {RRM_DARK};
      }}
      .rrm-card {{
        background: {RRM_LIGHT};
        border-left: 5px solid {RRM_RED};
        padding: 1rem 1.2rem;
        border-radius: 6px;
        margin: 0.6rem 0;
      }}
...
st.markdown("<h1 class='rrm-title'>Cemetery Health Check</h1>", unsafe_allow_html=True)
st.markdown(
    "<div class='rrm-sub'>A 60-second snapshot of how the cemetery shows up online.</div>",
    unsafe_allow_html=True,
)
...
with st.form("health_check_form", clear_on_submit=False):
    business = st.text_input("Cemetery Name", placeholder="e.g., Forest Lawn Memorial Park")
    home_city = st.text_input("Home City", placeholder="e.g., Glendale")
...
if st.session_state.get("scan_started") and not st.session_state.get("scan_done"):
    ...
    if missing:
        ...
        log_lines = []
        spinner_box.info(
            "Scanning... reviews, 3-Pack (cemetery + cemetery plots), SEO, ads, "
            "website running in parallel."
        )
...
if st.session_state.get("scan_done"):
    ...
    if results:
        ...
        st.markdown(
            f"<div class='rrm-card'>"
            f"<div style='font-size:0.9rem;color:{RRM_GRAY};text-transform:uppercase;letter-spacing:0.05em;'>Overall Grade</div>"
            f"<div style='font-size:3rem;font-weight:800;color:{RRM_DARK};line-height:1;'>{overall}</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
        ...
        with st.expander("Google 3-Pack", expanded=False):
            ...
            for label, d in (pack.get("results") or {}).items():
                top3 = ", ".join((d.get("top_3") or [])[:3]) or "—"
                if d.get("in_3_pack"):
                    st.write(f"**{label}:** Found at rank {d.get('rank')} (Top 3: {top3})")
                else:
                    st.write(f"**{label}:** Not found (Top 3: {top3})")

        with st.expander("Google SEO", expanded=False):
            for label, d in (results.get("seo") or {}).items():
                rank = d.get("rank")
                top3 = ", ".join((d.get("top_3") or [])[:3]) or "—"
                rank_txt = f"Rank {rank}" if rank else "Not in top 10"
                st.write(f"**{label}:** {rank_txt} (Top 3: {top3})")

        with st.expander("Google Ads", expanded=False):
            for label, d in (results.get("ads") or {}).items():
                advs = ", ".join(d.get("all_advertisers") or []) or "No ads detected"
                st.write(f"**{label}:** {advs}")
                if d.get("prospect_running_ads"):
                    st.write(f"&nbsp;&nbsp;↳ Prospect running ads in {label}")
        ...
        with st.expander("Website", expanded=False):
            ...
            if web.get("unverified"):
                ...
                for k, label in [
                    ...
                    ("family_owner_story", "Family/owner story"),
                    ("preneed_form", "Preneed form / schedule a tour"),
                    ("localbusiness_schema", "LocalBusiness schema"),
                    ("google_reviews_widget", "Google reviews widget"),
                    ("property_tour_photos", "Property tour photos"),
                    ("inventory_info", "Inventory / availability info"),
                ]:
                    v = checks.get(k)
                    mark = "✅" if v is True else ("❌" if v is False else "—")
                    st.write(f"- {label}: {mark}")
                inv = web.get("inventory_offerings") or []
                if inv:
                    st.write(f"&nbsp;&nbsp;↳ Offerings detected: {', '.join(inv[:6])}")
                pmap = web.get("property_map")
                if pmap is not None:
                    mark = "✅" if pmap else "❌"
                    st.write(f"- Property map (data point, not graded): {mark}")
